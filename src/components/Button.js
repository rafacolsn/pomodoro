...
export default class Button extends Component {
    ...
    render() {
        let up;
        let down;
        this.props.toggle ? up = <button className="button btn is-primary center is-light is-rounded"
            onClick={() => this.props.changeSession(this.props.session + 1)}>+1</button> :
            up = <button className="button btn is-danger center is-light is-rounded">+1</button>

        this.props.toggle ? down = <button className="button btn is-primary center is-light is-rounded"
            onClick={() => this.props.changeSession(this.props.session - 1)}>-1</button> :
            down = <button className="button btn is-danger center is-light is-rounded">-1</button>
